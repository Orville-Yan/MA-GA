...
def multi_regress(y, x_s):
    ...
    det = torch.det(w)
    singularity = ((det < down_epsilon) | (abs(det) >= up_epsilon)).unsqueeze(-1).unsqueeze(-1).expand(-1, x_s.shape[-1] + 1,
                                                                                            x_s.shape[-1] + 1)
...
class op:
    def __init__(self):
        self.variable1_func_list = ['at_log', 'at_sign', 'at_signsqrt', 'at_sigmoid','at_neg', 'cs_rank'
            , 'cs_scale', 'cs_zscore', 'cs_harmonic_mean', 'cs_edge_flip', 'cs_winsor', 'cs_demean','cs_distance2mean',]

        self.on_parameter_func_list = ['at_signpower', 'ts_harmonic_mean','ts_pctchg_abs','ts_pctchg','ts_delay', 'ts_delta', 'ts_mean', 'ts_std', 'ts_to_max',
        ...
        self.variable2_func_list = ['at_add', 'at_div', 'at_mul', 'at_sub', 'cs_norm_spread', 'cs_umr', 'cs_cut','cs_regress_res']
        self.variable3_func_list = ['ts_regress_res','ts_regress_k','ts_regress_b','ts_rankcorr','ts_correlation','ts_weight_mean']
