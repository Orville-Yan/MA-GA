...
class stratified:
    def __init__(self,factor_frame,factor_name,buydays,selldays,bins_num=5,plot=True):
        ...
        self.n= bins_num
        self.plot = plot
        # self.factor_frame = self.factor_frame.loc[self.selldays]
        # self.period = period # 回测周期
        factor_dates = self.factor_frame.index.unique().tolist()
        ...
        if len(factor_dates)!=len(selldays):
            raise CustomError(f'Length of factor unique dates is {len(factor_dates)} not equal to length of selldays {len(selldays)}')
    ...
    def get_every_interval_rate(self):
        ...
        self.every_interval_rate=pd.DataFrame(bins_return,index=list(range(self.n))+['ret_mean'],columns=self.selldays).T.dropna()
    ...
    def plot_backtest(self,root_path=None):
        ...
        long_annualized_ret = long_cum_ret.iloc[-1]**(252/len(long_cum_ret))
        short_annualized_ret = short_cum_ret.iloc[-1]**(252/len(short_cum_ret))
        long_mean_annualized_ret = long_mean_cum_ret.iloc[-1]**(252/len(long_mean_cum_ret))
        mean_short_annualized_ret = mean_short_cum_ret.iloc[-1]**(252/len(mean_short_cum_ret))
        mean_annualized_ret = mean_cum_ret.iloc[-1]**(252/len(mean_cum_ret))
        
        long_annualized_vola = long_ret.std()**np.sqrt(252)
        short_annualized_vola = short_ret.std()**np.sqrt(252)
        long_mean_annualized_vola = long_mean_ret.std()**np.sqrt(252)
        mean_short_annualized_vola = mean_short_ret.std()**np.sqrt(252)
        mean_annualized_vola = mean_ret.std()**np.sqrt(252)
        
        long_sharp = (long_ret-1).mean()/(long_ret-1).std()*np.sqrt(252)
        short_sharp = (short_ret-1).mean()/(short_ret-1).std()*np.sqrt(252)
        long_mean_sharp = (long_mean_ret-1).mean()/(long_mean_ret-1).std()*np.sqrt(252)
        mean_short_sharp = (mean_short_ret-1).mean()/(mean_short_ret-1).std()*np.sqrt(252)
        mean_sharp = (mean_ret-1).mean()/(mean_ret-1).std()*np.sqrt(252)
    ...
    def plot_IC(self,root_path):
        ...
        IC_p1 = self.factor.corrwith(self.p1_ret,method='pearson')
      
        fig = plt.figure(figsize=(18, 5))
        plt.plot(IC_p1.index,IC_p1.cumsum(),label=f'IC:{IC_p1.to_numpy().mean()/IC_p1.to_numpy().std()*np.sqrt(252)}')
        plt.title('2016-2023 IC')
        plt.legend()
        if self.plot:
            plt.show()
        else:
