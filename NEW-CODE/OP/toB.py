...
class op_BB2B:
    def __init__(self):
        self.func_list=['at_add','at_sub','at_div','cs_cut','cs_umr','at_prod','cs_norm_spread']
    ...
    @staticmethod
    def M_cs_cut( x, y):
        sign = op_BB2B.at_sign(x - op_BB2B.nanmean(x, dim=1).unsqueeze(1))
        return sign * y
    @staticmethod
    def M_cs_umr( x, y):
        s=torch.multiply(x - op_BB2B.nanmean(y, dim=1).unsqueeze(1), y)
        return torch.where((s==torch.inf)|(s==-torch.inf),float('nan'),s)
    @staticmethod
...
class op_BA2B: #B*A-B
    def __init__(self):
        self.variable1_func_list = ['M_toD_standard']  # B*A-B
...
class op_BG2B: #B*G-B
    def __init__(self):
        self.variable2_func_list = ['M_cs_edge_flip']  # B*G-B
