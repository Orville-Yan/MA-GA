...
class OP_A2A:
    def __init__(self):
        self.func_list=['D_at_abs','D_cs_rank','D_cs_scale','D_cs_zscore','D_cs_harmonic_mean','D_cs_demean','D_cs_winsor']
    @staticmethod
    def D_at_abs(x):#取绝对值
        return torch.abs(x)
...
class OP_AA2A:
    def __init__(self):
        self.func_list=['D_cs_norm_spread','D_cs_cut','D_cs_regress_res','D_at_add','D_at_sub','D_at_div','D_at_prod','D_at_mean']
...
class OP_AF2A:
    ...
    def D_ts_pctchg( x, lookback):
        s=(x-OP_AF2A.ts_delay(x,lookback))/OP_AF2A.ts_delay(x,lookback)
        return torch.where((s==torch.inf)|(s==-torch.inf),float('nan'),s)
    ...
    def D_ts_detrend(x, lookback):#ts_regress
        ...
        k, b, _ = OP_AAF2A.ts_regress(time_idx_expanded, x, lookback)
...
class OP_BBD2A:
    ...
    def D_Minute_area_bifurcate_mean(m_tensor_x,m_tensor_y,mask):
        OP_AA2A.D_at_sub(OP_BD2A.D_Minute_area_mean(m_tensor_x, OP_BA2D.Mmask_day_plus(m_tensor_y,OP_BD2A.D_Minute_area_mean(m_tensor_y, mask)))),
        OP_BD2A.D_Minute_area_mean(m_tensor_x, OP_BA2D.Mmask_day_sub(m_tensor_y,OP_BD2A.D_Minute_area_mean(m_tensor_y, mask)))))

    @staticmethod
    def D_Minute_area_bifurcate_std(m_tensor_x,m_tensor_y,mask):
        OP_AA2A.D_at_sub(OP_BD2A.D_Minute_area_std(m_tensor_x, OP_BA2D.Mmask_day_plus(m_tensor_y,OP_BD2A.D_Minute_area_mean(m_tensor_y, mask)))),
        OP_BD2A.D_Minute_area_std(m_tensor_x, OP_BA2D.Mmask_day_sub(m_tensor_y,OP_BD2A.D_Minute_area_mean(m_tensor_y, mask)))))
...
class OP_B2A:
    ...
    def D_minute_trend(x):
        ...
        time_idx = torch.arange(x.shape[0], dtype=torch.float32).unsqueeze(-1)
        time_idx_expanded = time_idx.repeat(1, x.shape[1])
        k, b, _ = OP_Basic.regress(time_idx_expanded, x)
